...
# required argument
parser.add_argument("AngNum", help="number of torsions, 0 < AngNum <= 26, labeled 'a' - 'z'", type=int)
# optional arguments
# preparation:
...
for torsion in all_torsion_angles:
    ...
    gaussian = open('spectrum_%s.dat' % torsion, 'w')  # convoluted spectrum
...
if args.Flexible:
    if args.Clustering:
	summary_classification = open('summary_classification_gk%d_t%d_%s_c.log'
    ...
    summary_classification = open('summary_classification_gk%d_t%d_c.log'
                                  % (args.GaussKern, args.Threshold), 'w')
else:
...
# frame id (no editing necessary, reference = last frame)
label_frameid_class_pos_tuple = zip(class_list_cent, range(1, len(class_list_cent)+1))
label_frameid_div_pos_tuple = zip(div_list_cent, range(1, len(div_list_cent)+1))
...
if args.Xray:
    ...
    #edit classid lists when reference included:
    class_list_id = [classification.class_centroids.index(CENT)+1 for CENT in class_list_cent[1:]]
    class_list_id.insert(0, "x")
    div_list_id = [classification.class_centroids.index(DIVCENT)+1 for DIVCENT in div_list_cent[1:]]
    div_list_id.insert(0, "x")

else:
    # class id without reference
    class_list_id = [classification.class_centroids.index(CENT)+1 for CENT in class_list_cent]
    div_list_id = [classification.class_centroids.index(DIVCENT)+1 for DIVCENT in div_list_cent]

# zip class ids with numbers for gnu
label_classid_class_pos_tuple = zip(class_list_id, range(1, len(class_list_id)+1))
label_classid_div_pos_tuple = zip(div_list_id, range(1, len(div_list_id)+1))

if args.Clustering:
	#frame id
	label_frameid_clus_pos_tuple = zip(clus_list_cent, range(1, len(clus_list_cent)+1))
	#class id
	if args.Xray:
	    clus_list_id = [classification.class_centroids.index(CLUSCENT)+1 for CLUSCENT in clus_list_cent[1:]]
	    clus_list_id.insert(0, "x")
	else:
	    clus_list_id = [classification.class_centroids.index(CLUSCENT)+1 for CLUSCENT in clus_list_cent]
	
	label_classid_clus_pos_tuple = zip(clus_list_id, range(1, len(clus_list_id)+1))
    
heatmap.write(
	"#----TICS---- \n" +
	"#change according to which subset is plotted, see plot, select either frame id or class id \n" +
	"#frame id of first " + str(args.SubSize) + " class centroids \n" +
        "set xtics(" + ' '.join("'%s' %s," % x for x in label_frameid_class_pos_tuple) +") rotate by 90 \n" +
	"set ytics(" + ' '.join("'%s' %s," % x for x in label_frameid_class_pos_tuple) +") \n" +
	"#class id of first " + str(args.SubSize) + " class centroids \n" +
        "#set xtics(" + ' '.join("'%s' %s," % x for x in label_classid_class_pos_tuple) +") rotate by 90 \n" +
	"#set ytics(" + ' '.join("'%s' %s," % x for x in label_classid_class_pos_tuple) +") \n" +
	"#frame id of " + str(args.SubSize) + " diverse class centroids \n" +
        "#set xtics(" + ' '.join("'%s' %s," % x for x in label_frameid_div_pos_tuple) +") rotate by 90 \n" +
	"#set ytics(" + ' '.join("'%s' %s," % x for x in label_frameid_div_pos_tuple) +") \n" +
	"#class id of " + str(args.SubSize) + " diverse class centroids \n" +
        "#set xtics(" + ' '.join("'%s' %s," % x for x in label_classid_div_pos_tuple) +") rotate by 90 \n" +
	"#set ytics(" + ' '.join("'%s' %s," % x for x in label_classid_div_pos_tuple) +") \n \n" )

if args.Clustering:
	heatmap.write(
	"#frame id of " + str(args.SubSize) + " clustered class centroids \n" +
        "#set xtics(" + ' '.join("'%s' %s," % x for x in label_frameid_clus_pos_tuple) +") rotate by 90 \n" +
	"#set ytics(" + ' '.join("'%s' %s," % x for x in label_frameid_clus_pos_tuple) +") \n" +
	"#class id of " + str(args.SubSize) + " clustered class centroids \n" +
        "#set xtics(" + ' '.join("'%s' %s," % x for x in label_classid_clus_pos_tuple) +") rotate by 90 \n" +
	"#set ytics(" + ' '.join("'%s' %s," % x for x in label_classid_clus_pos_tuple) +") \n \n" )
...
heatmap.write(
    ...
    "set label \"d-RMSD [{\260}]\" at 14,6 center rotate by -90 \n \n" +
    "#----PLOT---- \n" +
